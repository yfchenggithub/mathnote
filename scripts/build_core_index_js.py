...
--------
1. 全量自动发现模块并构建：
   `python scripts/build_core_index_js.py`
2. 只构建一个模块：
   `python scripts/build_core_index_js.py --module 07_inequality`
3. 只构建某个条目并查看它的索引展开结果：
   `python scripts/build_core_index_js.py --item I005 --debug-doc I005 --dry-run`
4. 检查某个查询词最终会命中哪些倒排项：
   `python scripts/build_core_index_js.py --debug-term 柯西不等式 --dry-run`
5. 输出更易读的 JS，并把调试信息嵌入 bundle：
   `python scripts/build_core_index_js.py --pretty --embed-debug`
...
LOGGER = logging.getLogger("build_core_index_js")
...
def write_bundle(bundle: Mapping[str, object], config: BuildConfig) -> None:
    ...
    header = [
        "// Auto-generated by scripts/build_core_index_js.py",
