\begin{proofbox}
	...
	\begin{description}[style=nextline, leftmargin=2.8em, labelsep=0.5em, itemsep=0.55em]
		\item[\textbf{步骤一（定义域对称性）：}]
			因为 $f(x)$ 的定义域包含 $[0,+\infty)$，所以对任意 $x\in\mathbb{R}$，都有 $|x|\ge 0$，从而 $|x|$ 在 $f$ 的定义域内，故 $y=f(|x|)$ 的定义域为 $\mathbb{R}$。
			\[
				D_{f(|x|)}=\mathbb{R}.
			\]
		...
		\item[\textbf{步骤二（偶函数验证）：}]
			设 $g(x)=f(|x|)$。对任意 $x\in\mathbb{R}$，有
			\[
				g(-x)=f(|-x|)=f(|x|)=g(x).
			\]
			\par\textbf{理由：} 因为 $|-x|=|x|$，所以 $g(-x)=g(x)$，由偶函数定义可知 $g(x)=f(|x|)$ 是偶函数。
